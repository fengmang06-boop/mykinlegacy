...
## Etsy Listing Write Guard

OAuth is prepared with `listings_w`, but the app still defaults to read-only. Live listing writes remain disabled unless all safeguards pass:

* `ETSY_READ_ONLY_MODE=false` and `ETSY_WRITE_APPROVED=true`
* Founder approval and CSO approval are both recorded
* A reviewed dry-run diff exists
* A rollback baseline exists
* No more than 3 listings are edited per day
* Only title, tags, and description opening are allowed
* Price, inventory, shipping, category, images, and image order are forbidden

Getting `listings_w` only makes future approved edits technically possible. It does not enable automatic Etsy modification.
